...
int getFloor(void) { return elevio_floorSensor(); }

int getStopButton(void) { return elevio_stopButton(); }

int getObstructionButton(void) { return elevio_obstruction(); }
...
void updateOrderArray(Elevator *p_elevator) {
  for (int buttonIndex = 0; buttonIndex < NUM_ORDER_BUTTONS; buttonIndex++) {
    ...
    // The first 4 rows in the order array correspond to the order buttons on
    // the elevator panel.
    if (buttonIndex < 4) {
      ...
      // The rest of the buttons correspond to the order buttons on the control
      // panel. subtracting 4 to get the correct floor when accessing button
      // value.
...
int orderAbove(Elevator *p_elevator) {
  ...
  // incrementing by 5 to check the floors above the elevators currently floor,
  // and accessing right elements in orderArray.
...
int orderOnCurrentFloor(Elevator *p_elevator) {
  ...
  int hallUpButton =
      p_elevator->orderArray[p_elevator->currentFloor + 4][UP_BUTTON];
  int hallDownButton =
      p_elevator->orderArray[p_elevator->currentFloor + 4][DOWN_BUTTON];
...
int shouldStop(Elevator *p_elevator) {
  ...
  int hallUpButton =
      p_elevator->orderArray[currentFloor + NUM_FLOORS][UP_BUTTON];
  int hallDownButton =
      p_elevator->orderArray[currentFloor + NUM_FLOORS][DOWN_BUTTON];
  int orderOnFloor = orderOnCurrentFloor(p_elevator);

  switch (currentDirection) {
  case DIRN_UP:
    if ((cabinButton == ON || hallUpButton == ON) ||
        (hallDownButton == ON & HigherOrder == NO_ORDERS)) {
      return STOP;
    }
    break;
  ...
  case DIRN_DOWN:
    if (cabinButton == ON || hallDownButton == ON) {
      return STOP;
    } else if (cabinButton == ON && LowerOrder == NO_ORDERS ||
               hallUpButton == ON && LowerOrder == NO_ORDERS) {
      return STOP;
    }
    break;
